...
function PlayerChat(args, player)
	...
	if words[1] == "/secretcrash" and not args.player:InVehicle() then
		Chat:Send(args.player, "You need to be in a plane or helicopter to do that!", Color(255, 255, 255))
		return false
	elseif words[1] == "/secretcrash" and PlaneVehicles[args.player:GetVehicle():GetModelId()] then
		if Weather == 2 then
			...
			elseif math.random() > 0.5 then
				...
				Chat:Broadcast(tostring(args.player).. "'s plane ran out of fuel!", Color(255, 255, 255))
			end
	...
	elseif words[1] == "/secretcrash" and HeliVehicles[args.player:GetVehicle():GetModelId()] then
		if Weather == 2 then
			...
			elseif math.random() > 0.5 then
				...
				Chat:Broadcast(tostring(args.player).. "'s helicopter ran out of fuel!", Color(255, 255, 255))
			end
			Chat:Broadcast("Last blackbox broadcast puts " ..tostring(args.player) .." at " ..tostring(Positionx).. ", " ..tostring(Positionz).. ".", Color(255, 255, 255))
		return false
		else
	...
	elseif words[1] == "/secretcrash" and not AirVehicles[args.player:GetVehicle():GetModelId()] then
		Chat:Send(args.player, "You need to be in a plane or helicopter to do that!", Color(255, 255, 255))
		return false
	else return true
	end
